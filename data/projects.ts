...
const projectEntries: Project[] = [
  {
    ...
    outcome:
      'During the outage successful submissions fell to a few hundred a day, and no new jobs reached Google for 27 days. Once the fixes and the quota change shipped, new job submissions resumed at 1.5 to 1.9 million a day, jobs indexed in Google recovered from 250,000 to 950,000, and about 17% of active jobs became eligible for Google listings again. The job sitemap went live in eight countries, and its database lookup got 70x faster, from 83.8 ms to 1.194 ms. Since recovery the pipeline has peaked at 71,023 submissions in a minute with only 7 rejections across 49,624 active minutes. After the removal gate deployed, 0 of 49,927 sampled removals were duplicates. The research also produced a sizing proposal that halves the time to clear the removal backlog, from 36 days to 18, and a concrete ask for longer data retention and a lag alert. Each fix exposed the next design flaw in a system that had grown without monitoring or maintenance, so recovery became a sequence of measured repairs rather than one patch.',
    metrics:
      'Indexed jobs recovered from 250K to 950K. 15.9M rejections diagnosed as a daily quota, not a rate limit. New job submissions from 0 to 1.5M+ a day. Job sitemaps live in 8 countries. 7 rejections in 49,624 active minutes since. ~17% of active jobs re-qualified.',
    stack: [
      'PostgreSQL',
      'Kafka',
      'Redis',
