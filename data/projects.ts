...
const projectEntries: Project[] = [
  {
    ...
    outcome:
      'Successful submissions per day rose 9.5x once the fixes and the quota change shipped, and about 17% of active jobs became eligible for Google listings again. The job sitemap went live in eight countries, and its database lookup got 70x faster, from 83.8 ms to 1.194 ms. Since recovery the pipeline has peaked at 71,023 submissions in a minute with only 7 rejections across 49,624 active minutes. After the removal gate deployed, 0 of 49,927 sampled removals were duplicates. The research also produced a sizing proposal that halves the time to clear the removal backlog, from 36 days to 18, and a concrete ask for longer data retention and a lag alert.',
    metrics:
      '15.9M rejections diagnosed as a daily quota, not a rate limit. 9.5x daily submissions after recovery. Job sitemaps live in 8 countries. 7 rejections in 49,624 active minutes since. ~17% of active jobs re-qualified.',
    stack: [
      'PostgreSQL',
      'Kafka',
      'Redis',
